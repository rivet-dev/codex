...
where
    ...
    R: Send + 'static,
{
    // wasm32-wasip1 has no OS threads, so the blocking thread pool can never run
    // queued tasks (they hang forever). Run the work as an ordinary task on the
    // current-thread runtime instead. codex's blocking ops (sqlite, fs) are short
    // and cooperative, so this is correct for the single-threaded VM.
    #[cfg(target_os = "wasi")]
    {
        // Single-threaded wasip1: there is no blocking pool. Run the work INLINE
        // (synchronously) so it completes before any other task can interleave —
        // avoiding the deadlock where a deferred blocking task waits on a task that
        // can't run. Then hand back an already-ready JoinHandle. spawn_blocking work
        // is synchronous (FnOnce()->R) and must not block on the runtime.
        let result = func();
